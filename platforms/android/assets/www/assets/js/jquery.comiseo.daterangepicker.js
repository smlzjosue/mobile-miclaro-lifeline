...
(function($, window, undefined) {
  ...
  function buildCalendar(classnameContext, options) {
    ...
    function init() {
      ...
      $self.datepicker(
        $.extend({}, options.datepickerOptions, { beforeShowDay: beforeShowDay, onSelect: onSelectDay })
      );
      updateAtMidnight();
    }
    ...
    function beforeShowDay(date) {
      var result = [
          true, // selectable
          range.start && (+date === +range.start || (range.end && range.start <= date && date <= range.end))
            ? 'ui-state-highlight'
            : '' // class to be added
        ],
        userResult = [true, '', ''];
  ...
  function buildDateRangePicker($originalElement, instance, options) {
    ...
    function render() {
      $container = $('<div></div>', {
        class: classname + ' ' + classname + '-' + sides[hSide] + ' ui-widget ui-widget-content ui-corner-all ui-front'
      })
        .append(
      ...
      if (options.autoFitCalendars) {
        ...
        if (initialWidth > maxWidth) {
          ...
          if (numberOfMonths !== initialNumberOfMonths) {
            autoFit.monthWidth =
              (initialWidth - $container.outerWidth(true)) / (initialNumberOfMonths - numberOfMonths);
          }
        } else {
          while (
    ...
    function formatRangeForDisplay(range) {
      ...
      return (
        ...
        (+range.end !== +range.start ? options.rangeSplitter + $.datepicker.formatDate(dateFormat, range.end) : '')
      );
    }
    ...
    function reposition() {
      $container.position({
        ...
        using: function(coords, feedback) {
          ...
          vFit =
            (vSide === BOTTOM &&
              feedback.element.top - feedback.target.top !== feedback.target.height + options.verticalOffset) ||
            (vSide === TOP &&
              feedback.target.top - feedback.element.top !== feedback.element.height + options.verticalOffset);
